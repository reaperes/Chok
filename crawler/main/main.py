...
def getConnection(ip, userName, pwd, DBName):
    conn = pymysql.connect(host=ip, unix_socket='/tmp/mysql.sock', user=userName, passwd=pwd, db=DBName, charset='utf8')
    return conn;
...
def searchTweet():
    return tweepy.Cursor(api.search, q="", rpp=100, geocode="37.504525,127.048902,1km", result_type="recent", include_entities=True, lang="ko").items()

def divideTweet(tweet):
    return twitter.morphs(tweet.text)
